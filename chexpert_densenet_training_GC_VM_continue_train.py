...
# Parameters to change for training continuation
model_exist = 'densenet_model_t8_gc_small_v4.mdl'
prev_best_perf = 3.954
logfile = 'history_v4.txt'
new_model = 'densenet_model_t8_gc_small_v5.mdl'
...
model_ft = load_models(model_exist)
...
dataloaders_dict = {x: torch.utils.data.DataLoader(data[x], batch_size=batch_size,
                                                   shuffle=False, num_workers=4) for x in ['train', 'valid', 'test']}
...
optimizer_ft = torch.optim.SGD(params_to_update, lr=1e-4, momentum=0.9)
...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False):
    ...
    best_loss = 9999
    best_performance = prev_best_perf
...
f.write(repr(hist))
f.close()
torch.save(model_ft.state_dict(), new_model)
